...
static int ftdi_xvc_read_bytes(ftdi_xvc_ctx *ctx, unsigned int len, unsigned char *buf) {
  int bytes_read = 0;
  int to_read = len;
  while (bytes_read < to_read) {
    int result = ftdi_read_data(&ctx->ftdi, buf + bytes_read, to_read - bytes_read);
    if (result < 0) {
      fprintf(stderr, "xvcd: %s : read error: %s\n", __FUNCTION__, ftdi_get_error_string(&ctx->ftdi));
      return -1;
    }
    bytes_read += result;
  }
...
ftdi_xvc_ctx *ftdi_xvc_create(unsigned int verbosity) {
  ftdi_xvc_ctx *ctx = calloc(1, sizeof(ftdi_xvc_ctx));
  if (!ctx)
    return NULL;
  ftdi_init(&ctx->ftdi);
...
int ftdi_xvc_open_device(ftdi_xvc_ctx *ctx, int vendor, int product,
  ...
  ftdi_set_interface(&ctx->ftdi, iface);
  if (ftdi_usb_open_desc(&ctx->ftdi, vendor, product, NULL, serial) < 0) {
    fprintf(stderr, "xvcd: %s : can't open device: %s\n", __FUNCTION__,
            ftdi_get_error_string(&ctx->ftdi));
    return -1;
  }
  ftdi_usb_reset(&ctx->ftdi);
  ftdi_set_latency_timer(&ctx->ftdi, 1);
  return 0;
}
...
uint32_t ftdi_xvc_set_tck_period(ftdi_xvc_ctx *ctx, uint32_t period_ns) {
  ...
  if (ftdi_write_data(&ctx->ftdi, buf, 3) != 3) {
...
int ftdi_xvc_init_mpsse(ftdi_xvc_ctx *ctx, unsigned int freq_hz) {
  ...
  ftdi_set_bitmode(&ctx->ftdi, 0x0B, BITMODE_BITBANG);
  ftdi_set_bitmode(&ctx->ftdi, 0x0B, BITMODE_MPSSE);
  // Flush any pending data from the device
  while ((res = ftdi_read_data(&ctx->ftdi, &byte, 1)) > 0)
    ;
  if (ftdi_write_data(&ctx->ftdi, buf, 7) != 7) {
...
int ftdi_xvc_shift_command(ftdi_xvc_ctx *ctx, unsigned int len,
  ...
  while (left) { // loop until there are data to handle
    ...
    while ((desc_pos < (MAX_DATA - 8)) &&    // We may generate up to 9 descriptors in a single iteration
           ...
           (left > 0)) {                     // loop until we may add a command to the current set
      if ((left > 7) && (buffer[cur_byte_pos] == 0)) { // No TMS and at least 1 byte to transmit
        ...
        if ((buffer[cur_byte_pos] == 0) && (left <= 7)) { // No TMS, bit shift of last bits
          ...
          for (j = 0; j < 8; j++) {
            ...
            ftdi_cmd[wr_ptr++] =
                ((buffer[cur_byte_pos] & (1 << j)) ? 0x01 : 0x00) |
                ((buffer[cur_byte_pos + nr_bytes] & (1 << j)) ? 0x80 : 0x00);
            left--;
            rd_len += 1;
            // Add the descriptor to the read descriptors
    ...
    if (ftdi_write_data(&ctx->ftdi, ftdi_cmd, wr_ptr) != wr_ptr) {
      return 1;
    }
    // Read the response
    if (ftdi_xvc_read_bytes(ctx, rd_len, ftdi_res) < 0) {
      return 1;
    }
